...
router.use((req, res, next) => {
  console.log("A requset is coming into api ...");
  next();
});
...
router.get("/student/:_student_id", (req, res) => {
  ...
  Course.find({ students: _student_id })
    ...
    .catch(() => {
      res.status(500).send("Can't find the courses.");
    });
});
...
router.post("/", async (req, res) => {
  ...
  if (req.user.isStudent())
    return res.status(400).send("Only instructor can do it.");
  ...
  try {
    ...
    res.status(400).send("Cant Saved.");
  }
});
